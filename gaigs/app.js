...
function render(){
  ...
  const profileMark=$('.profile-chip > span');if(profileMark)profileMark.innerHTML=state.user.avatarUrl?`<img src="${esc(state.user.avatarUrl)}" alt="">`:esc(state.user.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase());
...
const views={
  ...
  profile(){const initials=state.user.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();return `${pageHead('Personal Profile','Your identity, skills, services, communities and public impact - controlled by you.',`<button class="action-btn" data-action="editProfile">Edit profile</button>`)}<div class="dashboard-grid"><section class="card span-12"><div class="profile-banner"></div><div class="profile-info"><div class="profile-avatar">${state.user.avatarUrl?`<img src="${esc(state.user.avatarUrl)}" alt="${esc(state.user.name)}">`:esc(initials)}</div><div class="profile-copy"><h2>${esc(state.user.name)}</h2><p>${esc(state.user.city)}, ${esc(state.user.country)} · ${state.user.emailVerified?'Email verified':'Email verification pending'} · Identity ${esc(state.user.kycStatus||'pending')}</p><div class="skill-grid">${String(state.user.skills||'').split(',').filter(Boolean).map(x=>`<span class="skill-chip">${esc(x.trim())}</span>`).join('')||'<span class="skill-chip">Add your skills</span>'}</div></div><div><span class="tag ${state.user.accountMode==='sites'?'green':'gold'}">${state.user.accountMode==='sites'?'SHARED ACCOUNT':'DEVICE PREVIEW'}</span></div></div></section>${card('Impact dashboard',`<div class="metric-row"><div class="metric"><small>Community contributions</small><b>${state.posts.length}</b><em>Hash-receipted rewards</em></div><div class="metric"><small>Wallet</small><b>${Number(state.wallet?.available||0).toLocaleString()} GCR</b><em>Closed-loop credits</em></div><div class="metric"><small>Governance participation</small><b>${Object.keys(state.votes||{}).length}</b><em>Your own ballots</em></div></div>`,8)}${card('Communities',memberships(),4)}${card('Recent posts and missions',feedPosts(true),8)}${card('Privacy & local-first data',`<div class="briefing"><b>Device cache:</b> offline drafts and preferences stay on this device.<br><b>Shared records:</b> published profile, posts, votes and GCR receipts replicate to the secure service.<br><b>Identity:</b> raw CNIC is never displayed or written to the ledger.<br><br>Phones help with offline access; replicated shared infrastructure prevents a community from losing records when one phone is unavailable.</div>`,4)}</div>`},
...
function transactionTable(emergency=false){const tx=(emergency?state.transactions.slice(0,3):state.transactions);return `<div class="transaction-list">${tx.map(t=>{const amount=t.currency==='GCR'?`${Number(t.amount||0).toLocaleString()} GCR`:money(t.amount);return `<div class="transaction-item"><div class="activity-icon">${t.amount>0?'↓':'↑'}</div><div style="flex:1"><b>${t.desc}</b><p class="muted" style="margin:4px 0 0">${t.date} · ${t.proof}</p></div><b style="color:${t.amount>0?'var(--green)':'var(--text)'}">${t.amount>0?'+':''}${amount}</b></div>`}).join('')}</div><button class="ghost-btn" style="margin-top:10px" data-action="verifyChain">Verify receipt chain</button>`}
...
function bindModal(){
  ...
  const pf=$('#postCreateForm');if(pf)pf.addEventListener('submit',async e=>{
    ...
    if(window.gaigsApi?.active()){
      try{
        let uploadId=null;if(mediaFile){toast('Uploading evidence…');uploadId=(await window.gaigsApi.upload(mediaFile,'post')).upload.id}
        const result=await window.gaigsApi.createPost({scope,type,text,location,uploadId,lat:$('#postApproxLocation').checked?state.user.lat:null,lng:$('#postApproxLocation').checked?state.user.lng:null});
        state.posts.unshift(result.post);save();closeModal();render();navigate('feed');toast(result.post.reward?`Published. +${result.post.reward} GCR impact reward recorded.`:'Post published to the shared feed.');
      }catch(error){toast(error.message||'Post could not be published.')}return;
    }
    if (useFirebase) {
  ...
  const epf=$('#editProfileForm');if(epf)epf.addEventListener('submit',async e=>{e.preventDefault();e.stopImmediatePropagation();
    try{
      state.user.name=$('#profileName').value;state.user.city=$('#profileCity').value;state.user.country=$('#profileCountry').value;state.user.skills=$('#profileSkills').value;
      if(window.gaigsApi?.active()){
        const avatar=$('#profileAvatar')?.files?.[0];if(avatar){const uploaded=await window.gaigsApi.upload(avatar,'avatar');state.user.avatarUrl=uploaded.avatarUrl}
        const updated=await window.gaigsApi.updateProfile({name:state.user.name,city:state.user.city,country:state.user.country,skills:state.user.skills,bio:$('#profileBio')?.value||state.user.bio||'',locationPublic:Boolean($('#profileLocationPublic')?.checked),lat:state.user.lat,lng:state.user.lng});state.user=updated.user;
      }
      save();closeModal();toast('Profile updated. JARVIS matching refreshed.');render();
    }catch(error){toast(error.message||'Profile could not be updated.')}
  });
}
...
function editProfileForm() {
  return `<form id="editProfileForm" class="form-grid">
    <label>Profile picture<input id="profileAvatar" type="file" accept="image/*"></label>
...
$('#signupForm').addEventListener('submit',async e=>{
  ...
  try{
    toast('Creating your secure shared account…');
    const result=await window.gaigsApi.register({fullName:$('#signupName').value.trim(),phone:$('#signupPhone').value.trim(),email,password,cnic,country:$('#signupCountry').value,city:$('#signupCity').value});
    state.user=result.user;state.wallet=result.wallet;state.view='overview';save();render();toast(`Account created. Your wallet ${result.wallet.id} is ready with 100 GCR founding credits.`);
  }catch(err){toast(err.message||'Registration failed.');}
});
$('#loginForm').addEventListener('submit',async e=>{
  ...
  try{const result=await window.gaigsApi.login($('#loginEmail').value.trim(),$('#loginPassword').value);state.user=result.user;state.wallet=result.wallet;save();render();toast('Welcome back. Your shared account and wallet are connected.');}
  catch(err){toast(err.message||'Login failed.');}
});
...
$('#logoutBtn').addEventListener('click',async()=>{if(firebaseAvailable&&fbAuth.currentUser)await fbAuth.signOut();if(window.gaigsApi?.active())await window.gaigsApi.logout().catch(()=>{});localStorage.removeItem('gaigsUser');state.user=null;render()});
